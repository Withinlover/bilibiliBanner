  ...
  <div>
    <div className="fall-banner">
      <div className="animate-banner" ref="animateBanner">
        <div className="layer">
          <img :src="background" data-width="3000" data-height="250" alt="background" />
        </div>
        ...
        <div className="layer">
          <img :src="foreground" data-width="3000" data-height="250" alt="foreground" />
        </div>
        <div className="layer">
...
/* eslint-disable */
const animateBanner = ref(null);
const animateGirl = ref(null);
const initConfig = [
  {
    aspect: 1,
    blur: 4,
...
onMounted(() => {
  ...
  if (animateBanner.value) {
    /**
     * Event mouseenter
     */
    animateBanner.value.addEventListener(
      "mouseenter",
      (event) => {
        const { width } = animateBanner.value.getBoundingClientRect();
...
const makeBlink = async () => {
  if (animateGirl.value) {
    const img = animateGirl.value.childNodes[0];
    await sleep(50);
    img.src = girlNapping;
    await sleep(50);
    img.src = girlClosed;
 ...
 * @returns {object}
 */
const movementTemplate = (blur = 0, x = 0, y = 0, rotate = 0, isReset = false) => {
...
const InitBannerImages = (banner, parallax) => {
  ...
  if (banner.value.childNodes && banner.value.childNodes.length > 0) {
    Array.from(banner.value.childNodes).forEach((item, key) => {
...
const makeBlurMoveEffect = (img, key, parallax) => {
  ...
  img.style = movementTemplate(blur, x, initConfig[key].y, initConfig[key].rotate);
};
/**
 ...
 */
const initRectSmallScreen = (key, item) => {
